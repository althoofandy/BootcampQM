...
export class GoogleSheetService {
    ...
    sequelize;
    initialized = false;
    constructor(sheetId, sequelizeInstance) {
    ...
    async ensureSheetsExist(requiredSheets) {
        ...
        return this.sequelize.transaction(async (transaction) => {
            ...
            for (const data of transformedData) {
                ...
                if (!existing) {
                    ...
                    console.warn(`Duplicate found for ${identifierField}: ${identifierValue} - Skipping insert`);
                }
            }
            return insertedCount;
        });
    }
    async massUpdate(tableName) {
        await this.initialize();
        const rows = await this.getSheetData(tableName);
        if (rows.length === 0)
    ...
    async transformData(tableName, data) {
        const transformers = {
            users: async (row) => ({
