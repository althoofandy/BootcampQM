...
export default {
  up: async (queryInterface: QueryInterface) => {
    ...
    // 1. Get transactions
    const [transactions]: any = await queryInterface.sequelize.query(
      `SELECT id FROM transactions ORDER BY "createdAt" DESC LIMIT 2;`
    );
    ...
    const [products]: any = await queryInterface.sequelize.query(
      `SELECT id, name, price FROM products WHERE stock > 0 LIMIT 3;`
    );
